...
```{r}
train_df <- train_df %>% select(-row.names(drop_columns_less_20))
test_df <- test_df %>% select(-row.names(drop_columns_less_20))
```
...
# Aquí deberíamos ya montar el train y test finales, en principio debería hacer falta únicamente añadir la columna de SalePrice al train
```
```{r}
dummy_vars_train <- dummyVars(" ~ .", data = train_df)
dummy_vars_test <- dummyVars(" ~ .", data = test_df)

predict_dummy_train <- data.frame(predict(dummy_vars_train, newdata = train_df))
